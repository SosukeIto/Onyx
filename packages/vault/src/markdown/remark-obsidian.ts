...
function wikilinkNodes(
	...
	if (isEmbed) {
		...
		if (ext !== "" && ext !== "md") {
			...
			if (path === null) {
				return [
					element(
						"span",
						{ className: ["embed", "unresolved"], dataTarget: label },
						[text(label)],
					),
				];
			}
		...
		if (path === null) {
			return [
				element(
					"div",
					{ className: ["embed", "unresolved"], dataTarget: parts.target },
	...
	if (path === null) {
		return [
			element(
				"a",
				{ className: ["wikilink", "unresolved"], dataTarget: parts.target },
				[text(parts.alias ?? parts.target)],
			),
		];
	}
...
function tokenize(
	...
	nodes = replaceInText(nodes, HIGHLIGHT, (match) => [
		element("mark", {}, [text(match[1] ?? "")]),
	]);
	if (allowTags) {
...
function liftBlockEmbeds(node: WalkableParent): void {
	...
	for (let i = 0; i < children.length; i++) {
		...
		if (
			...
			(child.children[0]?.data as Record<string, unknown> | undefined)?.[
				BLOCK
			] === true
		) {
