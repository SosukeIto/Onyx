...
export const Route = createFileRoute("/_app/daily/$date")({
  ...
  loader: async ({ context, params }) => {
    const detail = await context.queryClient.ensureQueryData(dailyDetailOptions(params.date));
    if (!detail) {
      throw notFound();
    }
...
function DailyCalendarPanel({ date }: { date: string }) {
  ...
  const hasToday = todayCalendar.data?.days.some((day) => day.date === today) ?? false;
