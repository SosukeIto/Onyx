...
export const Route = createFileRoute("/_app/graph")({
  ...
  loader: ({ context, deps }) => context.queryClient.ensureQueryData(graphOptions(deps.center)),
...
function GraphRoute() {
  ...
  const selection = useMemo(() => {
    ...
    return {
      items: [toItem(node), ...view.nodes.filter((entry) => around.has(entry.id)).map(toItem)],
      node,
      total: around.size,
    };
  }, [selected, view]);
