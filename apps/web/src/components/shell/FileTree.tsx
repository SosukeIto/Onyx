...
function Node({ node, depth, activePath, expanded, onToggle, onOpen, activeRef }: NodeProps) {
  if (node.kind === "file") {
    const active = node.path === activePath;
    return (
      <button
        aria-current={active ? "page" : undefined}
        className={cx(ROW, active && "bg-brand-soft font-medium text-brand hover:bg-brand-soft")}
    ...
    <>
      ...
      >
        <IconChevron
          ...
          strokeWidth={1.6}
        />
        <IconFolder className="flex-none text-ink-faint" size={15} strokeWidth={1.6} />
        <span className="min-w-0 flex-1 overflow-hidden text-ellipsis whitespace-nowrap">
          {node.name}
        </span>
        {node.noteCount > 0 ? (
