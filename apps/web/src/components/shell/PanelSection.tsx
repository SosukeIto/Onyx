...
export function PanelSection({
  ...
  return (
    ...
    >
      <div className="flex h-[34px] flex-none items-center gap-2 px-3 text-ink-muted" title={label}>
        {icon}
        <span className="sr-only">{label}</span>
        {count === undefined ? null : (
          <span className="ml-auto text-ink-muted text-micro tabular-nums">{count}</span>
        )}
        {action ? (
          <span className={cx("flex items-center", count === undefined && "ml-auto")}>
            {action}
          </span>
        ) : null}
      </div>
