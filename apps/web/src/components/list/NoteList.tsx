 ...
 */
export function NoteList({ items, activePath, onOpen, leading, className }: NoteListProps) {
  if (items.length === 0) {
    return null;
  }
  return (
    <div className={cx("min-w-0", className)}>
      {items.map((item) => {
        ...
        const meta = item.count === undefined ? item.modified : String(item.count);
        return (
          <button
          ...
          >
            ...
            <span className={cx("min-w-0 flex-1 truncate", active ? "text-brand" : "text-ink")}>
              {item.title}
            </span>
            {item.folder ? (
            ...
            {meta ? (
              <span className="flex-none text-ink-faint text-micro tabular-nums">{meta}</span>
            ) : null}
          </button>
        );
      })}
