...
export function Calendar({
	...
	const selectedDay = selected?.startsWith(`${label}-`)
		? Number(selected.slice(8))
		: undefined;
	const todayDay = today?.startsWith(`${label}-`)
		? Number(today.slice(8))
		: undefined;
	const firstWithNote = days.length > 0 ? Number(days[0].date.slice(8)) : 1;

	const [focusDay, setFocusDay] = useState(
		selectedDay ?? todayDay ?? firstWithNote,
	);
							...
							return (
								<button
									...
									className={cx(
										...
										isSelected &&
											"bg-brand font-bold text-brand-contrast hover:bg-brand",
									)}
									key={date}
									onClick={() => {
										if (has) {
