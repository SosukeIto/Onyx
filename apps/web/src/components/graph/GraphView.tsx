...
/** Structural copy of `graphNodeSchema` in `packages/api/src/schemas.ts`. */
...
export function GraphView({
	...
	const visibleNodes = useMemo(
		() =>
			showUnresolved
				? nodes
				: nodes.filter((node) => node.kind !== "unresolved"),
	...
	useEffect(() => {
		...
		const simLinks: SimulationLinkDatum<SimNode>[] = visibleEdges.map(
	...
	return (
		<div
			className={cx(
				"relative min-h-0 min-w-0 flex-1 overflow-hidden bg-app",
				className,
			)}
			style={{
				backgroundImage:
		...
		>
			...
			>
				...
				<g>
					{visibleNodes.map((node) => {
						...
						const r = Math.max(
							2,
							baseRadius(node.inDegree, maxInDegree) * nodeScale,
						);
			...
			<div className="absolute bottom-3.5 left-3.5 flex items-center gap-4 rounded-lg border border-line bg-panel/90 px-3 py-2 text-ink-muted text-micro backdrop-blur-sm">
				...
				>
					<span
						aria-hidden="true"
						className="size-1.5 flex-none rounded-full bg-brand"
					/>
					<span
						aria-hidden="true"
						className="size-3 flex-none rounded-full bg-brand"
					/>
